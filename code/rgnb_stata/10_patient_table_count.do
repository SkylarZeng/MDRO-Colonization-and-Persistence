...
local cond1 "(first_valid_mac == "mac1" & mac1ID == 43) |"
local cond2 "(first_valid_mac == "mac2" & mac1ID == 43) |"
local cond3 "(first_valid_mac == "mac3" & mac1ID == 43) |"
local cond4 "(first_valid_mac == "mac4" & mac1ID == 43)"
...
// Save patient counts to a temporary file
tempfile patient_table
save `patient_table'
...
forvalues i = 1/3 {
    ...
    replace ctcr_visit_count = real(word("`ctcr_visit_counts'", `i')) if bucket_group_patient == `i'
    replace cipr_visit_count = real(word("`cipr_visit_counts'", `i')) if bucket_group_patient == `i'
    replace imir_visit_count = real(word("`imir_visit_counts'", `i')) if bucket_group_patient == `i'

    replace ctcr_patient_count = real(word("`ctcr_patient_counts'", `i')) if bucket_group_patient == `i'
    replace cipr_patient_count = real(word("`cipr_patient_counts'", `i')) if bucket_group_patient == `i'
    replace imir_patient_count = real(word("`imir_patient_counts'", `i')) if bucket_group_patient == `i'
}

// Merge with patient counts table
merge 1:1 bucket_group_patient using `patient_table', nogenerate

// Export the combined table to a single CSV file
export delimited bucket_group_patient swab_count ctcr_swab_count cipr_swab_count imir_swab_count ///
    visit_count ctcr_visit_count cipr_visit_count imir_visit_count ///
    patient_count ctcr_patient_count cipr_patient_count imir_patient_count ///
    using "combined_results.csv", replace
